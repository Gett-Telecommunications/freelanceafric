...
export class SellerCareerService {
  ...
  async getSellerCareerById(uid: string): Promise<I_SellerCareer | null> {
  ...
  async getSellerReviewCareerByID(uid: string): Promise<I_SellerCareer | null> {
    const docRef = doc(this.collection, uid, 'drafts', 'review');
    const docSnap = await getDoc(docRef);
    if (docSnap.exists()) {
      return docSnap.data() as I_SellerCareer;
    }
  ...
  async approveSellerCareer(uid: string, approvalStatus: 'approved' | 'rejected', message = ''): Promise<boolean> {
    ...
    const career = await this.getSellerReviewCareerByID(uid);
    if (!career) return true;
    try {
      let approve: I_SellerCareer['status'] = 'inactive';
      if (approvalStatus === 'approved') {
        approve = 'active';
      } else if (approvalStatus === 'rejected') {
        approve = 'inactive';
      }
      const newDoc: I_SellerCareer = {
        ...career,
      ...
      await setDoc(doc(this.collection, uid), newDoc, { merge: true });
      // add the new draft to the history collection
      await addDoc(collection(this.collection, uid, 'history'), newDoc);
      // delete the draft
      await deleteDoc(doc(this.collection, uid, 'drafts', 'review'));
      return true;
    } catch (error) {
      console.log(error);
      return false;
