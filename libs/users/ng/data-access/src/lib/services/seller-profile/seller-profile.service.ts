...
export class SellerProfileService {
  ...
  async getSellerReviewProfileByID(uid: string): Promise<I_SellerProfile | null | false> {
    try {
  ...
  async approveSellerProfile(
    ...
    const profile = await this.getSellerReviewProfileByID(uid);
    if (!profile) return true;
    try {
      let approve: I_SellerProfile['status'] = 'inactive';
      if (approvalStatus === 'approved') {
        approve = 'active';
      } else if (approvalStatus === 'rejected') {
        approve = 'inactive';
      }
      const newDoc: I_SellerProfile = {
        ...profile,
      ...
      // delete the draft
      await deleteDoc(doc(this.collection, uid, 'draft', 'review'));
      return true;
    } catch (error) {
      console.log(error);
      return false;
